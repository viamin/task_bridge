...
RSpec.describe "task_bridge:sync collection grouping", :full_options do
  # Tests for the sync_collection grouping logic in sync.rake.
  ...
  def group_items_into_collections(items_by_service)
    ...
    items_by_collection.delete(nil)
    collections = []
  ...
  describe "ungrouped items" do
    it "does not create a SyncCollection for incomplete items with matching titles" do
      ...
      expect(item1.sync_collection_id).to be_nil
      expect(item2.sync_collection_id).to be_nil
    end
