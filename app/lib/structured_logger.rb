...
class StructuredLogger
  ...
  def print_logs
    ...
    @existing_logs.each do |log_hash|
      puts format("%-#{@space_needed}s | %18s | %18s | %12d", log_hash["service"], log_hash.fetch("last_attempted", ""), log_hash.fetch("last_successful", ""), log_hash.fetch("items_synced", 0)) if options[:services].include?(log_hash["service"].delete(" ")) # rubocop:disable Lint/FormatParameterMismatch
    end
  end
